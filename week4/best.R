best <- function(state, outcome) {
        ...
        if(outcome=="heart attack"){ 
                ...
                tapply(out_come2[,11],out_come2$Hospital.Name,min,na.rm=TRUE)         
                range(out_come2[,11],na.rm=TRUE)
                which.min(out_come2[,11])
                a<-out_come2$Hospital.Name[which.min(out_come2[,11])]
                ch<-as.character(a)
                ch
        }
                ...
                out_come2<-subset(out_come,out_come$State==state)
                out_come2$Hospital.Name<-as.factor(out_come2$Hospital.Name)                 
                tapply(out_come2[,17],out_come2$Hospital.Name,min,na.rm=TRUE)         
                range(out_come2[,17],na.rm=TRUE)
                which.min(out_come2[,17])
                a<-out_come2$Hospital.Name[which.min(out_come2[,17])]
                ch<-as.character(a)
                ch
        }
                ...
                out_come2<-subset(out_come,out_come$State==state)
                out_come2$Hospital.Name<-as.factor(out_come2$Hospital.Name)                 
                tapply(out_come2[,23],out_come2$Hospital.Name,min,na.rm=TRUE)         
                range(out_come2[,23],na.rm=TRUE)
                which.min(out_come2[,23])
                a<-out_come2$Hospital.Name[which.min(out_come2[,23])]
                ch<-as.character(a)
                ch
        }
